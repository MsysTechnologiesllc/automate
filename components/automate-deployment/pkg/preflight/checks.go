...
func requiredPortsForConfig(skipShared bool, config *dc.AutomateConfig) []int {
	if config == nil {
		config = dc.DefaultAutomateConfig()
	}

	servicesToCheck, _ := deployment.ExpectedServiceIDsForConfig(config.GetDeployment()) // nolint:errcheck
	...
	for i := 0; i < acValue.NumField(); i++ {
		if v, ok := acValue.Field(i).Interface().(a2conf.A2ServiceConfig); ok {
			if hasServiceByName(servicesToCheck, v.ServiceName()) {
				ports := v.ListPorts()
				for _, p := range ports {
					portVal, _ := v.GetPort(p.Name) // nolint:errcheck
...
func DefaultPortCheck(skipShared bool, config *dc.AutomateConfig) Check {
	return PortCheck(requiredPortsForConfig(skipShared, config))
}
